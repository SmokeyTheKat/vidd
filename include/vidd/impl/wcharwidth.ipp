#ifndef __VIDD_WCHAR_WIDTH_IPP__
#define __VIDD_WCHAR_WIDTH_IPP__

namespace WCharWidth {

namespace Impl {

/// Autogenerated. 1 sub-table(s). Consult [`lookup_width`] for layout info.
constexpr unsigned char TABLES_0[256] = {
	,
};

/// Autogenerated. 19 sub-table(s). Consult [`lookup_width`] for layout info.
constexpr unsigned char TABLES_1[2432] = {
	,
};

/// Autogenerated. 243 sub-table(s). Consult [`lookup_width`] for layout info.
constexpr unsigned char TABLES_2[3888] = {
	,
};

constexpr std::size_t lookupWidth(WChar c, bool is_cjk) {
	std::size_t cp = c.value;

	unsigned char t1_offset = TABLES_0[cp >> 13 & 0xFF];

	unsigned char t2_offset = TABLES_1[128 * std::size_t(t1_offset) + (cp >> 6 & 0x7F)];

	unsigned char packed_widths = TABLES_2[16 * std::size_t(t2_offset) + (cp >> 2 & 0xF)];

	unsigned char width = packed_widths >> (2 * (cp & 0b11)) & 0b11;

	if (width == 3) {
		if (is_cjk) return 2;
		else return 1;
	} else {
		return width;
	}
}

}; // namespace Impl

constexpr std::size_t width(WChar c, bool is_cjk = true) {
	WChar utf32;
	if (c.length() == 1) {
		utf32.value = (c.bytes[0] & 0b01111111);
	} else if (c.length() == 2) {
		utf32.value = (c.bytes[0] & 0b00011111) << 6 | (c.bytes[0] & 0b00111111);
	} else if (c.length() == 3) {
		utf32.value = (c.bytes[0] & 0b00001111) << 12 | (c.bytes[1] & 0b00111111) << 6 | (c.bytes[2] & 0b00111111);
	} else {
		utf32.value = (c.bytes[0] & 0b00000111) << 18 | (c.bytes[1] & 0b00111111) << 12 | (c.bytes[2] & 0b00111111) << 6 | (c.bytes[3] & 0b00111111);
	}

	if (utf32.value < 0x7F) {
		if (utf32.value >= 0x20) return 1;
		else return 0;
	} else if (utf32.value >= 0xA0) {
		return Impl::lookupWidth(utf32, is_cjk);
	} else {
		return 0;
	}
}

}; // namespace WCharWidth

#endif
